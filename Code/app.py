# this file is responcible for the
...
def login():
    if user_name.get() == "" or password.get() == "":
        ...
        try:
            con = pymysql.connect(host="localhost", user="root", password="", database="stockDump")
            cur = con.cursor()
...
def deshboard():
    des = Tk()
    des.title("Admin Panel BackTrading App")
    ...
    con = pymysql.connect(host="127.0.0.1", user="root", password="", database="stockDump")
    cur = con.cursor()
    ...
    # Book Label
    Investor = Label(des, text="Investor:", font='Verdana 10 bold')
    Investor.place(x=480, y=145)
    ...
    Investor_box = ttk.Combobox(des, width=30, textvariable=Investor_var, state='readonly')
    Investor_box['values'] = ('Andy', 'Charlie', 'Shetal', 'Danish', 'Sunil')
    Investor_box.current(0)
    Investor_box.place(x=550, y=145)
    ...
    for book in rows:
        d1 = Label(des, text=f"Investor: {book[9]}", font='Verdana 10 bold')
        d1.place(x=20, y=300)
...
def signup():
    ...
    winsignup = Tk()
    winsignup.title("Investor Appointment App")
...
# app title
win.title("Investor Appointment App")
